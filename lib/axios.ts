...
export const apiService = {
  ...
  async reviewKycVerification(id: string, data: KycReview): Promise<KycVerification> {
    ...
    // Use founder override endpoints: approve or decline
    const isApproval = data.action === 'approve';
    const endpoint = isApproval
      ? `/api/founder/overrides/kyc/${id}/approve`
      : `/api/founder/overrides/kyc/${id}/decline`;

    // Different field names based on action
    const requestBody = isApproval
      ? { notes: data.reason || 'Approved by founder override' }
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.message || 'Failed to review KYC verification');
    }
